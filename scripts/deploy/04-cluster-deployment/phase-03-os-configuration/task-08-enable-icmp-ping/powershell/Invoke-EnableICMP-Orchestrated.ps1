...
.EXAMPLE
    .\Invoke-EnableICMP-Orchestrated.ps1 -ConfigPath ".\configs\infrastructure-azl-lab.yml"
    .\Invoke-EnableICMP-Orchestrated.ps1
#>
...
foreach ($node in $nodes) {
    ...
    try {
        $r = Invoke-Command -ComputerName $ip -Credential $cred -ScriptBlock {
            ...
            foreach ($ruleName in $rules) {
                ...
                if ($rule) {
                    ...
                    if ($rule.Enabled -ne "True") { $failed += $ruleName }
                } else {
                    $failed += "$ruleName (not found)"
                }
            }
            [PSCustomObject]@{
        ...
        $color = if ($r.Status -eq "PASS") { "SUCCESS" } else { "ERROR" }
        $detail = if ($r.Failed) { "  [$($r.Failed)]" } else { "" }
        Write-Log "[$hostname] $($r.Status)$detail" $color
